...
app = FastAPI(
    title="DSEX Backend",
    description="Dhaka Stock Exchange (DSEX) backend for OpenBB Workspace — v3.0.0",
    version="3.0.0",
)
...
def _flt(record: dict, *keys: str, default: float = 0.0) -> float:
    """Try multiple key variants, return first parseable float."""
    for k in keys:
...
def _normalize_live_records(records: list[dict]) -> list[dict]:
    """
    Map raw DSE live-price table rows (with keys like 'LTP*', 'CLOSEP*', 'YCP*')
    into clean standardized dicts for OpenBB widgets.

    DSE live pages typically have these headers:
    #  |  TRADING CODE  |  LTP*  |  HIGH  |  LOW  |  CLOSEP*  |  YCP*  |
    ...
    for r in records:
        ...
        pct    = round((change / ycp * 100), 2) if ycp else 0.0
...
def dsex_live_prices(
    ...
    """
    Live prices for all DSEX index constituents.
    Source: dseX_share.php (index-specific page — replaces latest_share_price_scroll_l.php).
    Cached 2 minutes. Header-based parsing replaces brittle column-index approach.
    """
    records = _fetch_live("DSEX")
...
def dsex_top_gainers(
    ...
    """Top DSEX gainers by % change today (from dseX_share.php)."""
    df = pd.DataFrame(_fetch_live("DSEX"))
    if df.empty:
        return []
...
def dsex_top_losers(
    ...
    """Top DSEX losers by % change today (from dseX_share.php)."""
    df = pd.DataFrame(_fetch_live("DSEX"))
    if df.empty:
        return []
